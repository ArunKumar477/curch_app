...
session_start();
						...
						foreach ( $mode as $mkey => $md ) {
							foreach ( $cheque as $chkey => $chq ) {
								if( ( $ckey == $akey ) && ( $akey == $cmkey ) && ( $crkey == $crkey ) && ( $crkey == $mkey )  && ( $mkey == $chkey) && ( $chkey == $ckey )){
									$chq = ($chq) ? $chq : 0;
									$query = "update offerings set offer_date = ".$off_date.",receipt_no = '".$_POST['receipt_no']."',name = '".$name."',mobile = ".$mobile.",gender ='".$gender."',age =".$age.",email ='".$email."',mtype =".$mtype.",amount = ".$amt.",cat_id ='".$cat."',address ='".$address."',cms_share = '".$cmshare."',chruch_share = '".$crshare."',updated_by = ".$_SESSION['userid'].",updated_at =".strtotime(date("Y-m-d H:i:s")) .",mof = '".$md."',cheque_no = '".$chq."', cid = ".$cid." where id = " .$_POST['oId'];
	...
	if( $chk !== 'Y' ){
		...
		$iquery = "select receipt_no from offerings where cid = ".$cid." order by id DESC limit 1";
					...
					foreach ( $church_share as $crkey => $crshare ) {
						foreach ( $mode as $mkey => $md ) {
								foreach ( $cheque as $chkey => $chq ) {
									if( ( $ckey == $akey ) && ( $akey == $cmkey ) && ( $crkey == $crkey ) && ( $crkey == $mkey )  && ( $mkey == $chkey) && ( $chkey == $ckey )){
										$chq = ($chq) ? $chq : 0;
									$query = "insert into offerings( cid,receipt_no,offer_date,name,mobile,gender,age,email,mtype,amount,cat_id,address,cms_share,chruch_share,created_by,created_at,cheque_no,mof ) values( ".$cid.",'".$receipt_no."',".$off_date.",'".$name."',".$mobile.",'".$gender."',".$age.",'".$email."',".$mtype.",".$amt.",'".$cat."','".$address."',".$cmshare.",".$crshare.",".$_SESSION['userid'].", ". strtotime(date("Y-m-d H:i:s")) .",'".$chq."','".$md."')";
									$exe = mysqli_query($con,$query);
									if( $exe ){
										$return = 1;
									}else{
